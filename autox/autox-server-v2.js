...
function handleScreenshot() {
    ...
    try {
        ...
        if (!img) return fail("截图失败: 无法捕获屏幕");

        // 方法1: 优先使用 images.toBase64 (最简单可靠)
        try {
            var b64Direct = images.toBase64(img, "jpg", 70);
            if (b64Direct) {
                try { img.recycle(); } catch (e) {}
                return ok({ base64: String(b64Direct), format: "jpeg", length: String(b64Direct).length });
            }
        } catch (e1) {
            console.log("[v2.1] toBase64 失败: " + e1);
        }
        ...
        try {
            ...
            console.log("[v2.1] getBitmap 失败: " + e2);
        }
        
        if (!bitmap && img.bitmap) {
            bitmap = img.bitmap;
        }
        
        if (!bitmap) {
            try { img.recycle(); } catch (e) {}
            return fail("截图失败: 无法获取 Bitmap");
        }
        ...
        return ok({ base64: b64Str, format: "jpeg", length: b64Str.length });
    } catch (e) { 
        return fail("截图异常: " + (e.message || String(e))); 
    }
}
...
console.log("[v2.1] HTTP 服务已启动，端口: " + PORT);
        ...
        try {
            ...
            threads.start(function () {
                ...
                try {
                    ...
                    console.error("[v2.1] 处理请求出错: " + e);
                } finally {
                    try { s.close(); } catch (e2) {}
                }
            });
        } catch (e) {
            if (running) console.error("[v2.1] accept 出错: " + e);
        }
    }
});
